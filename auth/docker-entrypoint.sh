...
if [ "$KC_HOSTNAME" = "0.0.0.0:8080" ]; then
  ...
  exec /opt/keycloak/bin/kc.sh start-dev --db-url "jdbc:postgresql://hornet-finder-keycloak-db/keycloak" --db postgres --db-username keycloak --db-password "$KC_DB_PASSWORD" --hostname "auth.localhost" --hostname-strict=false

...
- **Keycloak Admin API access**: a dedicated service account (`flow-admin@velutina.invalid`, realm-management roles on the app realm, not on master) is available for the Keycloak REST Admin API (auth flows, realm roles, groups, users). Credentials are in `.env`: `FLOW_ADMIN_PASSWORD` (and `FLOW_ADMIN_USERNAME` if set, otherwise the address above). Get a token with the password grant on the app realm (`hornet-finder` in prod, `hornet-finder-dev` in dev) via `https://$KC_HOSTNAME/realms/<realm>/protocol/openid-connect/token` (`client_id=admin-cli`), then call `https://$KC_HOSTNAME/admin/realms/<realm>/...`. Never print the secrets; read-only calls are fine, confirm with the user before any write, especially in prod
